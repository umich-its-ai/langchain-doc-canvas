...
class CanvasClient():
    ...
    def get_loaders(self, course: Course, index_external_urls: bool, logger: Logger) -> dict[str, BaseSectionLoader]:
        mivideo_loader = MiVideoLoader(canvas=self._canvas, course=course, indexed_items=self.indexed_items, logger=logger)
        base_vars = BaseSectionLoaderVars(canvas=self._canvas, course=course, indexed_items=self.indexed_items, logger=logger, mivideo_loader=mivideo_loader)
