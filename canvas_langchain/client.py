...
class CanvasClient():
    ...
    def get_available_tabs(self, course: Course) -> list[str]:
        return [tab.label for tab in course.get_tabs()]

    def get_loaders(self, course: Course, index_external_urls: bool, logger: Logger) -> dict[str, BaseSectionLoader]:
        base_vars = BaseSectionLoaderVars(canvas=self._canvas, course=course, indexed_items=set(), logger=logger)
